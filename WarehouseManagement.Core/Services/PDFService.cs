    ...
    public class PDFService : IPDFService
    {
        public byte[] GenerateBarcodePdfReport(DeliveryPDFModelDto model)
        {
            using (var pdfStream = new MemoryStream())
            {
                ...
                int num = 1;
                var receptionNumbers = model.ReceptionNumber.Split(" | ", StringSplitOptions.RemoveEmptyEntries).ToList();
                ...
                foreach (var number in receptionNumbers)
                {
                    ...
                    using (var barcodeImage = barcodeWriter.Write(number))
                    {
                        using (var ms = new MemoryStream())
                        {
                            ...
                            var barcodeImageElement = new iText.Layout.Element.Image(barcodeImageData);

                            // Add numeration
                            mainTable.AddCell(new Cell()
                                .Add(new Paragraph(num.ToString())
                                    .SetFont(headerFont)
                                    .SetFontSize(16))
                                .SetTextAlignment(TextAlignment.CENTER)
                                .SetVerticalAlignment(VerticalAlignment.MIDDLE));
